...
from core.intent_router import route_intent, UserIntent
...
class UnifiedOrchestrator:
    ...
    async def process_message(
        ...
        # 3. Intent Routing
        routing = route_intent(user_input, session_id)
        intent = routing.get("intent")
        
        # 4. Delegate based on intent
        if intent == UserIntent.FULL_PIPELINE.value:
            return await self._handle_full_pipeline(user_input, session_id, routing)
            
        elif intent == UserIntent.SINGLE_AGENT.value:
            return await self._handle_single_agent(user_input, session_id, routing)
    ...
    async def _handle_full_pipeline(self, user_input: str, session_id: str, routing: dict) -> OrchestratorResponse:
        """Trigger the main research pipeline."""
        company = routing.get("extracted_company") or user_input
        ticker = routing.get("extracted_ticker") or ""
    ...
    async def _handle_single_agent(self, user_input: str, session_id: str, routing: dict) -> OrchestratorResponse:
        """Trigger just one agent."""
        agent = routing.get("target_agent", "metrics")
        company = routing.get("extracted_company") or user_input
        ticker = routing.get("extracted_ticker") or ""
